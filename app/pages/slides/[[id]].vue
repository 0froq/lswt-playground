...
const currentIndex = computed(() => {
  ...
  if (!id || !slides.value) return -1
  return slides.value.findIndex((s: any) => s.id === id)
})

const currentSlide = computed(() => {
  if (currentIndex.value === -1 || !slides.value) return null
  return slides.value[currentIndex.value]
})

const prevSlide = computed(() => {
  if (!slides.value || currentIndex.value <= 0) return null
  return slides.value[currentIndex.value - 1]
})

const nextSlide = computed(() => {
  if (!slides.value || currentIndex.value >= slides.value.length - 1) return null
  return slides.value[currentIndex.value + 1]
})
